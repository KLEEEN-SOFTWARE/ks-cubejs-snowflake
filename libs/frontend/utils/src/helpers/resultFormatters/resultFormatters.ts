...
export function getColorForSeverityValues(
  ...
  if (SameSDTAggregations.includes(transformation as Transformation)) {
...
export function isAttributeNumericType(attribute: Attribute): boolean {
  const selfAggregations = [Transformation.SelfMulti, Transformation.SelfSingle];
  const isNotSelfAggregation =
    Boolean(attribute.aggregation) && !selfAggregations.includes(attribute.aggregation as Transformation);
