...
export function RelativeDateDropdown({ onFilter }: DatePickerSectionProps) {
  ...
  function handleFilter(relativeDate: IntervalDate) {
    const newFilterQuery = {
      ...filterQuery,
      rules: filterQuery.rules.reduce((acc, rule) => {
        if (isTimestampRelativeDateRule(rule)) {
          const timestampRelativeDateRuleUpdated = {
            ...rule,
            value: { displayValue: relativeDate },
          };

          acc.push(timestampRelativeDateRuleUpdated);
...
function isTimestampRelativeDateRule({ field, operation }: { field: string; operation: FilterOperation }) {
  return field === TIMESTAMP_INTERNAL_OBJECT_NAME && operation === FilterOperation.RelativeDate;
}
//#endregion
